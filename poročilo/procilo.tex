...
\section{Opis kode}
\medskip
V prilogi sva dodala najino kodo napisano v \textit{pythonu}, ki je ustrezno komentirana pri bolj zapletenih 
...
argumenta $\lambda$, ki označuje maksimalno število predmetov s spremenjeno težo ter $\bar{w} = max\_w$, ki prikazuje robustne teže predmetov. 
...
ne podamo neobveznih argumentov $\lambda$ in $max\_w$ nam funkcija \textit{solve\_RKP} vrne podobne optimalne vrednosti kot prej, le da tokrat 
reši KP.
\par
