...
argumenta $\Gamma$, ki označuje maksimalno število predmetov s spremenjeno težo ter $max\_w := \bar{w}$, ki prikazuje robustne teže predmetov. 
...
ki sprejme argumente $N, z^*, k^*, c^*,  \Gamma, w, \overline{w}, p$ in neobvezen argument 
...
delnic po njihovi maksimalni ceni ob danem proračunu lahko kupimo in temu prilagodi sezname $p$,
$maks\_p$ in $r$. Recimo, da je naš proračun enak $10$ in maksimalna cena delnice $i$ enaka
...
da bo vrednost opazovanih delnic narasla. Nato sva s pomočjo funkcije $random.binomial(stevilo\_delnic, p)$ 
